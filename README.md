...
- **Expand all / Collapse all**: buttons in the graph panel to reset every node's collapse state at once, instead of one node at a time.
- **Value coloring**: field values are colored by inferred type (string/number/boolean/null), independent of the theme's default text color -- a legend at the bottom of the graph panel explains which color is which.
- **Collapse the editor**: the "‹" handle between the two panels hides the editor so the graph takes the full width; "›" brings it back.
